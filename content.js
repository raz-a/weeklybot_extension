...
(function () {
    ...
    class Logger {
        ...
        loadSettings() {
            chrome.storage.sync.get(["debugLog"], (result) => {
                this.debugEnabled = result.debugLog || false;
                this.log("Debug logging initialized:", this.debugEnabled ? "ENABLED" : "DISABLED");
            });
        }
        ...
        setupStorageListener() {
            chrome.storage.onChanged.addListener((changes, namespace) => {
                if (namespace === "sync" && changes.debugLog) {
                    this.debugEnabled = changes.debugLog.newValue;
                    this.log(
                        "Debug logging changed to:",
        ...
        logError(...args) {
            console.error("[Extension Debug]", ...args);
        }
    }
    ...
    function modifyWeeklyBotMessage(chatMessage) {
        try {
            ...
            const usernameElement =
                chatMessage.querySelector('[data-a-target="chat-message-username"]') ||
            ...
            if (!usernameElement) {
                logger.log("No username element found in message");
                return;
            }

            const username = usernameElement.textContent.trim();
            if (username !== "Weekly_Bot") {
                logger.log("Message is not from Weekly_Bot, skipping modification");
                return;
            }

            logger.log("Found Weekly_Bot message!");

            // Multiple selectors for message content
            const messageElement =
                chatMessage.querySelector('[data-a-target="chat-message-text"]') ||
            ...
            if (!messageElement) {
                logger.log("No message content element found");
                return;
            }
            ...
            if (words.length === 0) {
                logger.log("Message has no words");
                return;
            }

            const firstWord = words[0];
            logger.log("First word:", firstWord);

            // Check if the first word is contained within 【 and 】
            if (!firstWord.startsWith("【") || !firstWord.endsWith("】")) {
                logger.log("First word is not wrapped in 【】, leaving message unchanged");
                return;
            }
            ...
            logger.log("Target username from brackets:", targetUsername);

            // Don't modify if the extracted username is empty
            if (targetUsername.length === 0) {
                logger.log("Username within brackets is empty");
                return;
            }

            // Modify the username display
            usernameElement.textContent = targetUsername;
            logger.log("Changed username to:", targetUsername);
            ...
            logger.log("Updated message content to:", remainingMessage);
            ...
            logger.log("Successfully modified message!");
        } catch (error) {
    ...
    function modifyUserBadges(chatMessage, targetUsername) {
        try {
            ...
            const originalUsernameText = usernameElement
                ? usernameElement.textContent
                : targetUsername;

            logger.log("Saved username text:", originalUsernameText);
            ...
            for (const selector of badgeSelectors) {
                ...
                if (badgeContainer) {
                    logger.log("Found badge container with selector:", selector);
                    break;
                }
            }
            ...
            if (!badgeContainer) {
                const individualBadges = chatMessage.querySelectorAll(
                    '.chat-badge, [data-a-target*="badge"], img[alt*="Moderator"], img[alt*="Subscriber"]'
                );
                if (individualBadges.length > 0) {
                    badgeContainer = individualBadges[0].parentElement;
                    logger.log("Found badge container via individual badges");
                }
            }
            ...
            if (!badgeContainer) {
                if (usernameElement) {
                    ...
                    badgeContainer =
                        usernameElement.previousElementSibling ||
                        usernameElement.parentElement.querySelector('[class*="badge"]')
                            ?.parentElement ||
                        usernameElement.parentElement;
                    logger.log("Using username element area for badges");
                }
            }

            if (!badgeContainer) {
                logger.log("No badge container found, creating one");
                ...
                if (usernameElement) {
                    ...
                    logger.log("Created new badge container");
                } else {
                    logger.log("Could not find username element to attach badges");
                    return;
                }
            }

            // Remove all existing badges from the container and nearby areas, but preserve username
            const existingBadges = chatMessage.querySelectorAll(
                '.chat-badge, [data-a-target*="badge"], img[alt*="Moderator"], img[alt*="Subscriber"], .tw-tooltip-wrapper'
            );
            existingBadges.forEach((badge) => {
                ...
                if (badge !== usernameElement && !badge.contains(usernameElement)) {
                    logger.log(
                        "Removing existing badge:",
                        badge.className || badge.alt || badge.tagName
                    );
                    badge.remove();
                }
            });
            ...
            if (
                badgeContainer !== usernameElement &&
                badgeContainer !== usernameElement?.parentElement
            ) {
                badgeContainer.innerHTML = "";
                logger.log("Cleared badge container");
            }
            ...
            logger.log("Added custom W badge");
            ...
            if (currentUsernameElement) {
                ...
                if (currentUsernameElement.textContent !== targetUsername) {
                    ...
                    logger.log("Restored username text to:", targetUsername);
                }
            } else {
                // If username element was accidentally removed, recreate it
                logger.log("Username element missing, recreating...");
                ...
                if (badgeContainer.nextSibling) {
                    badgeContainer.parentNode.insertBefore(
                        newUsernameElement,
                        badgeContainer.nextSibling
                    );
                } else {
                    badgeContainer.parentNode.appendChild(newUsernameElement);
                }
                logger.log("Recreated username element");
            }
        } catch (error) {
            logger.logError("Error modifying badges:", error);
        }
    }
    ...
    function observeChat() {
        ...
        for (const selector of possibleSelectors) {
            ...
            if (chatContainer) {
                logger.log("Found chat container with selector:", selector);
                break;
            }
        }
        ...
        if (!chatContainer) {
            ...
            if (messageElements.length > 0) {
                chatContainer =
                    messageElements[0].closest(".simplebar-content") ||
                    messageElements[0].parentElement;
                logger.log("Found chat container by finding existing messages");
            }
        }

        if (!chatContainer) {
            logger.log("Chat container not found with any selector, retrying...");
            logger.log("Available elements:", {
        ...
        logger.log("Chat container found:", chatContainer);
        ...
        messageSelectors.forEach((selector) => {
            ...
            logger.log(
                `Found ${existingMessages.length} existing messages with selector "${selector}"`
            );
                    ...
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        ...
                        for (const selector of messageSelectors) {
                            if (node.matches && node.matches(selector)) {
                                logger.log("New message detected with selector:", selector);
        ...
        logger.log("MutationObserver initialized for chat monitoring");
        logger.log("Extension fully loaded and watching for Weekly_Bot messages");
    }
    ...
    new MutationObserver(() => {
        if (location.href !== currentUrl) {
            ...
            logger.log("URL changed, reinitializing chat observer");
            setTimeout(observeChat, 2000);
        }
